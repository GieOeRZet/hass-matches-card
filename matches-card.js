...
class MatchesCard extends HTMLElement {
  setConfig(config) {
    ...
    this.config = {
      ...
      table: { zebra: true, separator: true, system_colors: true },
      ...config,
    };
  }
  ...
  set hass(hass) {
    ...
    const style = `
      <style>
        ...
        .dual-cell>*{flex:1;display:flex;align-items:center;justify-content:center;}
        ...
        .result-circle{border-radius:50%;width:${this.config.icon_size.result}px;height:${this.config.icon_size.result}px;
          color:white;display:flex;justify-content:center;align-items:center;font-weight:bold;margin:0 auto;}
    ...
    const rows = matches.map(match => {
      ...
      const leagueIcon =
        match.league === "L" ? "/hacsfiles/matches-card/logo/ekstraklasa.png" :
        match.league === "PP" ? "/hacsfiles/matches-card/logo/puchar.png" : null;
      ...
      const gradientCSS =
        this.config.view_mode === "gradient"
          ? `background: linear-gradient(to right,
              ...
              ${match.result ? this.config.colors[match.result].replace(")", `, ${this.config.gradient.alpha})`) : "rgba(0,0,0,0)"} 100%);`
          : "";
...
// ============================================================================
//  REJESTRACJA KARTY – powrót do oryginalnej nazwy
// ============================================================================
